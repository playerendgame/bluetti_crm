...
export default {
    ...
    methods: {
        ...
        refreshProducts() {
            axios.get('/ajax/admin/products/api').then(({data}) => this.products = data.data);
        },
        toggleEdit() {
        ...
        initializeOrder() {
            ...
            if(this.order.region_id){
                this.order.region = this.region.find(a => a.id === this.order.region_id) || null;
            }else {
                this.order.region = null;
            }
            if(this.order.province_id){
                this.order.province = this.province.find(a => a.id === this.order.province_id) || null;
            }else{
                this.order.province = null;
            }
            if(this.order.city_id){
                this.order.city = this.city.find(a => a.id === this.order.city_id) || null;
            } else {
