...
class OrderController extends Controller
{
    public function list(Request $request)
    {
        ...
        foreach ($orders as $order) {
            ...
            //     return (($order_product->quantity * $order_product->cogs));
            // });
        ...
        if (isset($input["is_new_customer"]) && $input["is_new_customer"] == 0) {
            ...
            if ($customer->save()) {
                ...
                if ($order_product->save()) {
                    ...
                    $purchaseOrders = PurchaseOrder::where('product_id', $order_product->product_id)
                        ->orderBy('id', 'asc')
                        ->get();
                
                    $remainingQuantity = $order_product->quantity;
                    $totalCogs = 0; // Initialize the total COGS

                    $totalStocksLeft = (clone $purchaseOrders)->sum('stocks_left');

                    if ($totalStocksLeft >= $product_order["quantity"]) {
                        foreach ($purchaseOrders as $index => $purchaseOrder) {
                            $deductedStocks = min($purchaseOrder->stocks_left, $remainingQuantity);
                            ...
                            $remainingQuantity -= $deductedStocks;
